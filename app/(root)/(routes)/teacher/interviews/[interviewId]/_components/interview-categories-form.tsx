...
import { CategoryKind, Interview } from "@prisma/client";
...
interface InterviewCategoriesFormProps {
    initialData: Interview;
    interviewId: string;
    options: { label: string; value: string }[];
}

export const InterviewCategoriesForm = ({
    initialData,
    interviewId,
    options,
}: InterviewCategoriesFormProps) => {
    ...
    const formSchema = z.object({
        categoryIDs: z
            ...
            .refine((value) => value.some((item) => item), {
                message:
                    language.interviewCategoryField
    ...
    const onAddCategory = async (
        ...
        try {
            ...
            toast.success(language.interviewCategoryField.categoryCreated);
            ...
            if (axios.isAxiosError(error) && error.response?.status === 409) {
                toast.error(
                    language.interviewCategoryField.categoryAlreadyExists
                );
            } else {
                toast.error(language.somethingWentWrong);
            }
                ...
                <Button onClick={toggleEdit} variant="ghost">
                    {isEditing ? (
                        ...
                        <>
                            ...
                            {language.interviewCategoryField.editCategory}
                        </>
                    )}
                </Button>
            </div>
            ...
            {!isEditing && selectedOptions.length !== 0 ? (
                ...
                !isEditing && (
                    <p className="text-sm mt-2 text-slate-500 italic">
                        {language.interviewCategoryField.noCategory}
                    </p>
                )
            )}
                    ...
                    <div className="mt-4 pt-4 border-t">
                        {!isAddingCategory ? (
                            ...
                            >
                                ...
                                {language.interviewCategoryField.addCategory}
                            </Button>
                        ) : (
