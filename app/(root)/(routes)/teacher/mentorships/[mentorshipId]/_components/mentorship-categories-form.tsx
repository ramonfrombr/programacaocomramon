...
import { CategoryKind, Mentorship } from "@prisma/client";
...
interface MentorshipCategoriesFormProps {
    initialData: Mentorship;
    mentorshipId: string;
    options: { label: string; value: string }[];
}

export const MentorshipCategoriesForm = ({
    initialData,
    mentorshipId,
    options,
}: MentorshipCategoriesFormProps) => {
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        try {
            await axios.patch(`/api/mentorships/${mentorshipId}`, values);
            toast.success(language.mentorshipUpdated);
            toggleEdit();
            router.refresh();
        } catch {
    ...
    const onAddCategory = async (
        ...
        try {
            ...
            toast.success(language.mentorshipCategoryField.categoryCreated);
            ...
            if (axios.isAxiosError(error) && error.response?.status === 409) {
                toast.error(
                    language.mentorshipCategoryField.categoryAlreadyExists
                );
            } else {
                toast.error(language.somethingWentWrong);
            }
    ...
    return (
        <div className="mt-6 border bg-slate-100 roudned-md p-4">
            <div className="font-medium flex items-center justify-between">
                {language.mentorshipCategoryField.mentorshipCategory}
                <Button onClick={toggleEdit} variant="ghost">
                    {isEditing ? (
                        ...
                        <>
                            ...
                            {language.mentorshipCategoryField.editCategory}
                        </>
                    )}
                </Button>
            </div>
            ...
            {!isEditing && selectedOptions.length !== 0 ? (
                ...
                !isEditing && (
                    <p className="text-sm mt-2 text-slate-500 italic">
                        {language.mentorshipCategoryField.noCategory}
                    </p>
                )
            )}
                    ...
                    <div className="mt-4 pt-4 border-t">
                        {!isAddingCategory ? (
                            ...
                            >
                                ...
                                {language.mentorshipCategoryField.addCategory}
                            </Button>
                        ) : (
